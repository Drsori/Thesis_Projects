...
rho = 10;       % volume-averaged density
...
% Cn = (0.0625)*((1+tanh(3*x/eps)).*(1+tanh(3*(1-x)/eps)).*(1+tanh(3*x/eps)).*(1+tanh(3*(1-x)/eps)));

R = sqrt((x-0.5).^2 + (y-0.21).^2);
Cn = 0.5*(1-tanh((R-0.2)/(2*eps*sqrt(2))));

contourf(X,Y,reshape(Cn,size(X))); shading interp, drawnow
colormap(jet)
% rnd = -1 + (2)*rand(N,1);
...
%     U_star(north) = x(north) - x(north).^2;
