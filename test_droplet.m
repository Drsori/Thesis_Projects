...
rho1 = 1.6;
rho2 = 1;
rho_star = rho2;          % volume-averaged density
...
Cn = tanh((0.2-sqrt((x-0.5).^2+(y-2.4).^2))/(sqrt(2)*eps))+tanh((2-y)/(sqrt(2)*eps)) + 1;
...
for i=1:Nt-1
    ...
    %%
    U_star = U - dt*U.*(Dx*U) - dt*V.*(Dy*U) + eta*(dt/rho_star)*DD*U - (dt/rho_star)*Cn.*(Dx*mu);
    
    V_star = V - dt*U.*(Dx*V) - dt*V.*(Dy*V) + eta*(dt/rho_star)*DD*V - (dt/rho_star)*Cn.*(Dy*mu) - (rho/rho_star - 1);
